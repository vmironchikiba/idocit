...
class LoadPayload {
  /// Returns a new [LoadPayload] instance.
  LoadPayload({
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LoadPayload &&
  ...
  /// Returns a new [LoadPayload] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LoadPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LoadPayload[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LoadPayload[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<LoadPayload> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LoadPayload>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LoadPayload.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LoadPayload> mapFromJson(dynamic json) {
    final map = <String, LoadPayload>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LoadPayload.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LoadPayload-objects as value to a dart map
  static Map<String, List<LoadPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LoadPayload>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LoadPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
